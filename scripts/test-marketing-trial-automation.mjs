...
assert('cron wires automation processor on daily path', /processMarketingTrialAutomation/.test(cronSrc) && /marketing-trial-automation/.test(cronSrc))
assert('cron weekly branch returns before automation', /event\.cron === '0 9 \* \* 1'/.test(cronSrc) && /Do not run marketing trial automation here/.test(cronSrc))
assert('cron schedules unchanged in source comments/branches', /0 9 \* \* 1/.test(cronSrc) && /Daily 10am UTC/.test(cronSrc))
