...
assert('billing banner verifies plan via API helper', /verifyUpgradeFromApi|showUpgradeVerificationBanner/.test(billingHtml) && /payment is (still )?being confirmed|Confirming your subscription/.test(billingHtml))
...
assert('dashboard verifies upgraded against bizPlan', /wantedUpgrade && paid|api\/billing\/status/.test(dashHtml) && /payment is being confirmed|Confirming your subscription/.test(dashHtml))
